...
- **RoomEnvironment** — The primary environmental container. A world-layer that isolates the viewport and establishes the spatial context for everything rendered inside it.
...
- **Environmental Surfaces** — Primitive surface components that form the physical vocabulary of the environment.
...
`RoomEnvironment` establishes a fully isolated viewport context. Scroll behavior, overflow, and spatial boundaries are all contained within the world boundary. The outside of the screen does not exist while you are inside the Room.
...
The neutral state — the current baseline — represents a `{ temperature: "neutral", pacing: "steady", tone: "balanced" }` climate. Every rendering decision traces back to this model.
...
Even interactive primitives carry atmospheric intent. Buttons in Solen are not `primary` or `secondary` — they are `settle`, `proceed`, or `observe`. These intents map to token values that fit the environmental context.
...
This is v0.1.0. The following are not implemented:
...
- **Multi-world architecture** — RoomEnvironment is the first world. Others will follow, each with distinct climate models and atmospheric characters.
...
> Solen uses [React](https://react.dev/) and is bundled with [Turbopack](https://nextjs.org/docs/app/api-reference/turbopack/).  
> Next.js `>=16` required.
