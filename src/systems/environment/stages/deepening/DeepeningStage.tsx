...
import { resonanceFragments } from "@/worlds/room/data/resonanceFragments";
import {
  presenceThresholds,
  type PresenceThreshold,
} from "@/worlds/room/data/presenceThresholds";
  ...
  return (
    <div className="relative">
      <div className="absolute top-4 left-4 z-20 sm:top-6 sm:left-6">
        <p className="text-sm tracking-wide opacity-15 text-[hsl(var(--text-ambient))]">
          solen
        </p>
      </div>
      ...
      <AtmosphericStack className="deepening-stage">
        ...
        <EnvironmentReveal phase={reveal.getPhase("deepening-silence")}>
          <div className="deepening-silence h-8" aria-hidden="true">
          </div>
        </EnvironmentReveal>

        {/* LAYER 2: PERIPHERAL EMOTIONAL RESIDUE */}
        <EnvironmentReveal phase={reveal.getPhase("deepening-residue")}>
          <div className="deepening-residue flex flex-col gap-3 ">
        ...
        <div className="flex flex-col">
          ...
          <EnvironmentReveal phase={reveal.getPhase("deepening-title")}>
            <h1 className="heading">
              you can move deeper, if you want to.
            </h1>
          </EnvironmentReveal>
        </div>
        ...
        <EnvironmentReveal phase={reveal.getPhase("deepening-thresholds")}>
          <div className="deepening-thresholds flex mt-10 flex-col items-start gap-4 sm:flex-row sm:items-stretch sm:justify-center sm:gap-6">
            {presenceThresholds.map((threshold) => (
              <ThresholdItem
                key={threshold.id}
                label={threshold.label}
                path={threshold.path}
                isSelected={selectedThreshold === threshold.id}
                onSelect={() => handleThresholdSelect(threshold.id)}
              />
            ))}
          </div>
        </EnvironmentReveal>
        ...
        {isPanelOpen && selectedThreshold === "self" && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-[hsl(var(--surface-base))] bg-opacity-95 p-4 backdrop-blur-sm transition-opacity duration-800">
            <div className="mx-auto w-full max-w-[min(100%,42rem)] rounded-lg border border-[hsl(var(--border-base))]/[0.1] bg-[hsl(var(--surface-base))] p-6 sm:p-8">
    ...
    {isPanelOpen && selectedThreshold === "resonance" && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-[hsl(var(--surface-base))] bg-opacity-95 p-4 backdrop-blur-sm transition-opacity duration-800">
        <div className="mx-auto max-h-[80vh] w-full max-w-[min(100%,42rem)] overflow-y-auto rounded-lg border border-[hsl(var(--border-base))]/[0.1] bg-[hsl(var(--surface-base))] p-6 sm:p-8">
          <div className="flex items-center justify-between mb-6">
    ...
    {isPanelOpen && selectedThreshold === "human" && (
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-[hsl(var(--surface-base))] bg-opacity-95 p-4 backdrop-blur-sm transition-opacity duration-800">
        <div className="mx-auto w-full max-w-[min(100%,28rem)] rounded-lg border border-[hsl(var(--border-base))]/[0.1] bg-[hsl(var(--surface-base))] p-6 text-center sm:p-8">
          <div className="flex items-center justify-between mb-6">
